...
export default class Notifications extends Component {
	...
	constructor( props ) {
		...
		store.subscribe( () => {
			this.setState( {
				notifications: [ ...store.getState().notifications[ this.location ] ],
			} );
		} );
	}
		...
		return ( this.state.notifications.length
			? this.state.notifications.map( ( notify, k ) => (
				<Notice
					...
					onDismiss={ () => delay( 100 ).then(
						() => store.dispatch( removeNotice( { location: notify.location, key: k } ) )
					) }
				/> ) )
			: null
		);
	...
	render() {
		...
		return (
			<div className={ classes }>
				...
				{ ( this.props.image || ( ! this.props.image && this.props.location !== 'dashboard' ) ) && (
					<div className="wp-notification-image">
						<img src={ this.props.image || wpLogo } alt={ this.props.title + ' image' } />
					</div>
				) }
			</div>
		);
